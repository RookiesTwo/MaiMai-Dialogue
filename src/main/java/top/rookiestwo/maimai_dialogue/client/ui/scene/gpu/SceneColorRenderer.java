...
public final class SceneColorRenderer {
    ...
    static void draw(GLTexture source, GLTexture target, ColorAdjustFilter settings) {
        ...
        try (var saved = new GlState()) {
            ...
            try {
                if (glCheckFramebufferStatus(GL_DRAW_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
                    throw new IllegalStateException("Scene color framebuffer is incomplete");
                ...
                glUniform3f(adjustmentsLocation, settings.brightness(), settings.contrast(), settings.saturation());
                int tint = settings.tint().map(value -> value.argb()).orElse(0);
                ...
                try { glDrawArrays(GL_TRIANGLES, 0, 3); }
                finally {
                    if (measure) { glEndQuery(GL_TIME_ELAPSED); pending[cursor] = true; cursor = (cursor + 1) % queries.length; }
                }
                if (!reported) {
                    reported = true;
                    MaiMaiDialogue.LOGGER.info("Scene GPU ColorAdjust: ordered pass executed, source={}x{}, output={}x{}, settings={}",
                            source.getWidth(), source.getHeight(), target.getWidth(), target.getHeight(), settings);
                }
            } finally {
    ...
    private static void collectTimings() {
        for (int i = 0; i < queries.length; i++) {
            ...
            if (++samples % 300 == 0 && diagnostics) MaiMaiDialogue.LOGGER.info("Scene GPU ColorAdjust: pass avg={} us, samples={}", Math.round(gpuMicros), samples);
        }
    }
    ...
    public static void shutdown() {
        ...
        framebuffer = vertexArray = program = 0; hookReady = false; reported = false;
    }
    private static String source(String file) {
        try (var stream = SceneColorRenderer.class.getResourceAsStream("/assets/maimai_dialogue/shaders/scene_color/" + file)) {
    ...
    private static int program(String fragment) {
        ...
        try {
            vertex = shader(GL_VERTEX_SHADER, "fullscreen.vert"); pixel = shader(GL_FRAGMENT_SHADER, fragment);
    ...
    private static final class GlState implements AutoCloseable {
        ...
        final int activeTexture = glGetInteger(GL_ACTIVE_TEXTURE), sampler = glGetIntegeri(GL_SAMPLER_BINDING, 0);
        ...
        GlState() {
            ...
            glActiveTexture(GL_TEXTURE0); texture = glGetInteger(GL_TEXTURE_BINDING_2D);
        ...
        @Override public void close() {
            ...
            glActiveTexture(GL_TEXTURE0); glBindTexture(GL_TEXTURE_2D, texture); glBindSampler(0, sampler); glActiveTexture(activeTexture);
