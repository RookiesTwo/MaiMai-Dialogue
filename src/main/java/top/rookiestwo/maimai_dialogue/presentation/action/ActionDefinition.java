...
public sealed interface ActionDefinition
        permits ActionDefinition.Reference, ActionDefinition.Inline {
    Codec<ActionDefinition> CODEC = Type.CODEC.dispatch(
            "type",
    ...
    record Reference(ResourceLocation id) implements ActionDefinition {
    ...
    record Inline(PresentationAction action) implements ActionDefinition {
        private static final MapCodec<Inline> CODEC =
                RecordCodecBuilder.mapCodec(instance -> instance.group(
                        PresentationAction.CODEC.fieldOf("action")
    ...
    enum Type {
        ...
        private final MapCodec<? extends ActionDefinition> codec;

        Type(
                String serializedName,
                MapCodec<? extends ActionDefinition> codec
        ) {
            this.serializedName = serializedName;
            this.codec = codec;
        }

        public MapCodec<? extends ActionDefinition> codec() {
            return codec;
        }
