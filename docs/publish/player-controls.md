...
- 默认按住左侧或右侧 Ctrl 时，文字和场景动画以 4 倍速度播放；Continue Step 就绪后会自动播放下一步，直到进入 EndStep。EndStep 不会自动退出，松开 Ctrl 后恢复正常速度。
...
"入口对话""返回""关闭"这些概念的整体规则见[会话与导航](../concepts/session.md)。
