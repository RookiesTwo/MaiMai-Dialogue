...
<!-- TODO(截图): 标注对话界面各控件：右上角跳过按钮（长按圆环）、History 入口、Expand 按钮、选项区 -->
...
- 默认按住左侧或右侧 Ctrl 时，文字和场景动画以 4 倍速度播放；当前页播完后会自动进入下一页，直到进入结尾页。结尾页不会自动退出，松开 Ctrl 后恢复正常速度。
...
- 点击 Dialogue target 时会再次检查最新的 Progress 状态。
...
- 作者配置 `dialogue` Exit 时，结尾页播放完成后仍会停留，玩家再次推进才进入目标 Dialogue。
- Esc 和界面关闭操作会结束当前对话。
