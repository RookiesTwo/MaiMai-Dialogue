...
把完整文件同步保存到两个 Pack。`steps` 按数组顺序播放，`end` 始终是最后一页。

### 第一次进游戏会遇到的"按了没反应"

正文是逐字显示的。**有文字还在播放时，第一次推进只会把文字立刻显示完整，不会翻页；再按一次才进入下一页。** 动画还在播时也是同样的规则。

```mermaid
flowchart LR
    A[文字正在播放] -->|第一次推进| B[文字立即显示完整]
    B -->|第二次推进| C[进入下一页]
```

这条规则会在每一步重复，所以连续推进时感觉是"按一下出一页"；只有刚进入新一步、内容还没播完时，才需要按两下。这是正常设计，不是 bug。
...
该值表示每个字之间等待的毫秒数，范围为 `0..1000`，默认 `30`。数值越小越快；`0` 表示不显示打字机效果、文字立即出现。配置只作用于所在节点，不会改变同一 Dialogue 中的其他步骤。
