...
八个参数的 `0` 均关闭对应效果。`chromatic_aberration` 使用场景实际渲染像素作为偏移单位。曲率扭曲场景采样坐标；噪点和闪烁随时间变化。CRT 视口默认黑底，曲率造成的空隙与素材透明区域显示为黑底，不透出游戏画面。

`bloom > 0` 时增加亮部提取和两次模糊，在宽高各为原场景四分之一的纹理上执行；`bloom = 0` 时仅执行 CRT 主 pass。八个参数全为 `0` 时绕过滤镜处理，但保留 CRT 黑底；移除 CRT 后黑底随之移除。
