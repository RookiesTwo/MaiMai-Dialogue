...
和自定义动画文件一样，在步骤的 `actions` 里用 `reference` 调用：
...
- 淡入与退场预设假定对象默认 `opacity` 为 `1.0`（视觉对象默认值）。若对象透明度不是 1，请先用直接写在对话里的动画恢复，否则退场预设的最终透明度可能低于 0。
- `variant` 是内容相关的单次切换，没有通用预设；切换差分请使用直接写在对话里的动画。
