---
title: 资源路径与 ID
description: 查询 Dialogue、Speaker、Theme、VisualAsset、SceneAction 和图片的目录映射规则。
---
...
- Speaker、Theme、VisualAsset、SceneAction 与图片只由客户端加载。
...
- `F3 + T`：重新加载客户端 Dialogue、Speaker、Theme、VisualAsset、SceneAction 和图片。

修改 Dialogue 时通常需要同时执行两种 reload。客户端 reload 后会集中报告缺失 Theme、Speaker、VisualAsset、目标 Dialogue、Action、target、variant 和图片。
