...
| `steps` | 否 | `[]` | 顺序播放的 ContinueStep |
| `end` | 是 | — | 唯一 EndStep，必须包含 `exit` |

## ContinueStep 与 EndStep
...
EndStep 额外要求 `exit`。省略 `text` 可制作纯动画 Step；不要使用 JSON `null`。
...
目标 Dialogue 会在选项显示前查询服务端，点击时再校验一次。条件不满足的选项会隐藏；若过滤后没有可见选项，当前 EndStep 按 Return fallback 处理。
